import torch
import sys
sys.path.append('D:\GitHub\RE\\1_算法示例\config_utils')
sys.path.append('D:\GitHub\RE\\')
...
def test():
    ...
    # 加载模型参数
    PATH_REL = 'D:\GitHub\RE\models\\rel_cls\\1m-acc0.79ccks2019_rel.pth'

    config_rel = ConfigRel()
    config_rel.batch_size = 8
    rel_model = BertForSequenceClassification.from_pretrained('D:\GitHub\RE\\bert-base-chinese', num_labels=config_rel.num_relations)
    # rel_model = AttBiLSTM(config_rel)
    rel_model_dict = torch.load(PATH_REL)
    rel_model.load_state_dict(rel_model_dict['state_dict'])
    # rel_test_path = './deploy/test.json' 
    rel_test_path = './data/test_data.json' 
