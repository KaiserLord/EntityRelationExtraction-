...
def get_embedding_pre():
    ...
    word2id = {}
    with codecs.open('D:\GitHub\RE\data\\vec.txt', 'r', encoding='utf-8') as f:
        cnt = 0
...
if __name__ == '__main__':
    ...
    config = ConfigRel()
    model = BertForSequenceClassification.from_pretrained('D:\GitHub\RE\\bert-base-chinese', num_labels=config.num_relations)
    data_processor = DataPreparationRel(config)
    train_loader, dev_loader, test_loader = data_processor.get_train_dev_data(
        'D:\GitHub\RE\data\\train_data.json', 
        'D:\GitHub\RE\data\dev_data.json', 
        'D:\GitHub\RE\data\\test_data.json')
